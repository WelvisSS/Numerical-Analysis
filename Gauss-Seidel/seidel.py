...
def seidel(n, A, B, epsilon, iter):
    ...
    for k in range(1, iter+1):
        ...
        for i in range(n):
            ...
            x[i] = B[i] - somatorio
        # Fazendo o cálculo da norma entre x que é o resultado obtido através da divisão 
        # de b e a diagonal principal e v.
        result = norma(x, v)
        # se a norma for menor ou igual a precisão, termina o processo retornando x.
        if result <= epsilon: return x
        # Caso contrário, v recebe x e o processo é feito novamente 
...
file_out.write(str(seidel(len(A) ,A, B, epsilon, 50)) + '\n')
